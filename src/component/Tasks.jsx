  ...
  return (
    <section>
        ...
        {tasks.length > 0 && (<ul className='p-4 mt-8 rounded-md text-white'>
            {tasks.map((task)=> <li className='flex justify-between my-4 text-white bg-blue-300 p-1 rounded-sm' key={task.id}>
                <span>{task.text}</span>
                <button className='text-white hover:text-red-700' onClick={()=>onDeleteTask(task.id)}>Clear</button>
            </li>)}
</ul>)} 
    </section>
  )
