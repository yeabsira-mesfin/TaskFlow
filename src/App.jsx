...
function App() {
 ...
 const selectedProject = projects.projects.find(project => project.id === projects.selectedProjectID)
  ...
  if(projects.selectedProjectID === null){
    content = <NewProject onAdd={handleProjectAdd} cancelProject ={cancelProject}/>
  } else if(projects.selectedProjectID === undefined) {
    content = <NoProjectSelected onStartedProject= {handleStartedProject} />
  }
  return (
    <main className="h-screen my-8 flex gap-8">
      <ProjectSidebar onSelecetProject = {handleSelectProject} onStartedProject ={handleStartedProject} projects = {projects.projects}/>
      {content}
    </main>
  );
}
